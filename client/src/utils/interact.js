...
export const mintNFTnoIPFS = async (url, name, description) => {
    //error handling
    if (url.trim() === "" || (name.trim() === "" || description.trim() === "")) {
        return {
            success: false,
    ...
    const metadata = new Object();
    metadata.name = name;
    metadata.image = url; //image part essential ? 
    metadata.description = description;

    //load smart contract
    const contract = new web3.eth.Contract(contractABI, contractAddress);

    const address = window.ethereum.selectedAddress; //get the address at teh selected address
    const nonce = await web3.eth.getTransactionCount(address);
